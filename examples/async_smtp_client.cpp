...
class client {
public:
	client(io_context& ioc, mail::mime::entity<mail::mime::string_body> m)
		: session_{ ioc }
		, mail_{ std::move(m) }
	{
	}
	...
	{
		async_connect(session_.next_layer(), eps,
					  [this](auto ec, auto) {
			if (ec) {
...
private:
	void open()
	{
		session_.async_open([this](auto ec) {
			if (ec) {
	...
	mail::smtp::session<ip::tcp::socket> session_;
