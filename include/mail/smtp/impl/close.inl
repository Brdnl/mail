...
namespace mail::smtp {
	namespace detail {
		inline auto quit_buffer()
		{
			return boost::asio::const_buffer{ "QUIT\r\n", 6 };
		}
	}

	template <class Stream>
	template <class Handler>
	class session<Stream>::close_op
	...
	public:
		...
		close_op(const close_op&) = delete;
		...
		friend bool asio_handler_is_continuation(close_op* op)
		{
	...
	template <class Stream>
	template <class Handler>
	void session<Stream>::close_op<Handler>::operator()(boost::beast::error_code ec, std::size_t bytes)
	{
		auto& d = *d_;
		BOOST_ASIO_CORO_REENTER(*this) {
			BOOST_ASIO_CORO_YIELD
				boost::asio::async_write(d.s.s_, detail::quit_buffer(), std::move(*this));
			if (ec) {
				goto upcall;
			}
			...
			if (d.s.resp_parser_.get().code() != reply_code::service_closing) {
				ec = error::failed;
				goto upcall;
			}
		upcall:
	...
	template <class Stream>
	void session<Stream>::close(boost::beast::error_code& ec)
	{
		static_assert(boost::beast::is_sync_stream<next_layer_type>::value,
					  "SyncStream requirements not met");

		boost::asio::write(s_, detail::quit_buffer(), ec);
		if (ec) {
			return;
		}
		read_resp(ec);
		...
		if (resp_parser_.get().code() != reply_code::service_closing) {
			ec = error::failed;
			return;
		}
	}
	...
	) session<Stream>::async_close(CloseHandler&& handler)
	{
